...
class TestGetEvents:
    ...
    @pytest.mark.asyncio
    async def test_get_running_events(
        self,
        mock_db,
        event_data,
        running_event_data,
        ...
        with patch.object(
            Event,
            "running",
            new_callable=lambda: property(lambda self: self.code == "running-event"),
        ):
...
class TestGenerateAndUploadQR:
    ...
    async def test_generate_and_upload_qr(self, mock_db, mock_blob_service):
        ...
        # Execute
        await _generate_and_upload_qr(
            event_id, event_code, mock_blob_service, service_url, mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_and_upload_qr_container_exists(
        self, mock_db, mock_blob_service
    ):
...
class TestUpdateEvent:
    ...
    @pytest.mark.asyncio
    async def test_update_event_simple_fields(
        self, mock_db, mock_blob_service, event_data, update_event_input
    ):
        # Setup mock
        event = Event(**event_data)

        with patch(
            "app.events.service.get_event", return_value=event
        ) as mock_get_event:
            # Execute
            ...
            mock_get_event.assert_called_once_with(
                mock_db, update_event_input.event_code
            )
    ...
    @pytest.mark.asyncio
    async def test_update_event_code(
        self, mock_db, mock_blob_service, event_data, update_event_code_input
    ):
        # Setup mock
        event = Event(**event_data)
        mock_result = MagicMock()
        ...
        with patch(
            ...
            result = await update_event(
                mock_db, update_event_code_input, mock_blob_service
            )

            # Assert
            mock_get_event.assert_called_once_with(
                mock_db, update_event_code_input.event_code
            )
            ...
            mock_blob_service.delete_container.assert_called_once_with(
                event_data["code"].lower()
            )

    @pytest.mark.asyncio
    async def test_update_event_code_conflict(
        self, mock_db, mock_blob_service, event_data, update_event_code_input
    ):
        ...
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = Event(
            code=update_event_code_input.new_event_code
        )
        mock_db.execute.return_value = mock_result
    ...
    @pytest.mark.asyncio
    async def test_update_event_integrity_error(
        self, mock_db, mock_blob_service, event_data, update_event_input
    ):
        # Setup mock
        event = Event(**event_data)
...
class TestUpsertEventImage:
    ...
    @pytest.mark.asyncio
    async def test_upsert_event_image(
        self, mock_db, mock_container, mock_upload_file, event_data
    ):
        # Setup mock
        event = Event(**event_data)
        ...
        with patch(
            ...
            assert (
                result.event_image_url == f"{mock_container.url}/assets/event_image.jpg"
            )
...
class TestDeleteEvent:
    ...
    @pytest.mark.asyncio
    async def test_delete_event_container_not_found(
        self, mock_db, mock_blob_service, event_data
    ):
        # Setup mock with container not found error
        event = Event(**event_data)
        mock_blob_service.delete_container.side_effect = ResourceNotFoundError(
            "Container not found"
        )
