...
class TestGetImageDetail:
    ...
    @pytest.mark.asyncio
    async def test_get_image_detail_success(
        self, mock_async_session, sample_image, sample_face
    ):
...
class TestDoFaceRecognition:
    ...
    def test_do_face_recognition_multiple_faces(
        self, mock_face_recognition, mock_pil_image
    ):
        ...
        mock_face_recognition.face_encodings.return_value = [[0.1] * 128, [0.2] * 128]
...
class TestFullProcessingJob:
    ...
    @pytest.mark.asyncio
    async def test_full_processing_job_success(
        self, mock_async_session, mock_container_client
    ):
        ...
        with patch("app.images.service.get_event", return_value=mock_event), patch(
    ...
    @pytest.mark.asyncio
    async def test_full_processing_job_event_not_found(
        self, mock_async_session, mock_container_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_full_processing_job_azure_upload_failure(
        self, mock_async_session, mock_container_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_full_processing_job_different_extensions(
        self, mock_async_session, mock_container_client
    ):
...
class TestDeleteImage:
    ...
    async def test_delete_image_success(
        self, mock_async_session, mock_container_client, sample_image
    ):
    ...
    async def test_delete_image_not_found(
        ...
        with pytest.raises(HTTPException) as excinfo:
            await delete_image(
                mock_async_session, mock_container_client, "nonexistent-uuid"
            )
    ...
    @pytest.mark.asyncio
    async def test_delete_image_blob_not_found(
        self, mock_async_session, mock_container_client, sample_image
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_image_url_parsing(
        self, mock_async_session, mock_container_client
    ):
        ...
        for case in test_cases:
            ...
            mock_container_client.delete_blob.assert_called_with(
                case["expected_blob_name"]
            )
